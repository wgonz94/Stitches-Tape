...
class NewMeasurement extends Component {
  ...
  // Capture information input into Measurement Form by Keyboard
  handleChange = e => {
    this.setState({
  ...
  // Submit measurement data and send user back to main Dashboard page
  handleSubmit = e => {
    e.preventDefault();
    ...
    this.props.history.push("/display/dashboard");
  };
      ...
      <main>
        <div className='scroll' style={{ backgroundColor: theme.bgc }}>
          <form id='NMForm' onSubmit={this.handleSubmit}>
            ...
            <div className='row'>
              {this.state.measurements.map((measure, index) =>
                measure.id === "units" ? (
                  <div className='switch'>
                    <label>
                      ...
                      <input
                        ...
                        id={measure.id}
                        checked={this.state.measurements.value}
                        key={measure[index]}
                      />
                ...
                ) : measure.id === "colorPref" || measure.id === "notes" ? (
                  <div className='input-field col s12 center'>
                    <Speech name={measure.id} handleVoice={this.handleVoice} />
                    <textarea
                      name={measure.id}
                      ...
                      rows={10}
                      placeholder=' '
                      value={this.state.measurements.value}
                      key={measure[index]}
                    />
                    <label htmlFor={measure.id}>{measure.name}</label>
                  </div>
                ) : (
                  <div className='input-field col s12 m6' key={measure[index]}>
                    <input
                      type='text'
                      onChange={this.handleChange}
                      ...
                      value={this.state.measurements.value}
                    />
                    <label htmlFor={measure.id} className='active'>
...
export default withRouter(NewMeasurement);
